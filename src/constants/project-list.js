export default [
  {
    id: 0,
    tag: 'Docsify',
    list: [
      {
        title: 'Docsify',
        layout: 'Docsify',
        api: 'Docsify',
        feature: `
                  目前正在使用的筆記管理工具，輕量且運行快速，可以更專注在文件產出，同時樹狀結構更容易查閱。
                  除了完善新的筆記，同時逐步將舊的筆記遷移到 Docsify
                `,
        github: 'https://github.com/wuzhe0912/docsify-note',
        link: 'https://docsify-pitt.netlify.app/',
        tags: ['Docsify', '筆記', '靜態站點'],
      },
    ],
  },
  {
    id: 1,
    tag: 'Vue.js',
    list: [
      {
        title: 'Vue.js 模仿 Web App',
        layout: '手刻 Flexbox',
        api: 'News API、六角旅館預約服務 API',
        feature: '仿 Web App 建議手機開啟，串接 News API、六角提供的旅館預約服務 API，實作小功能i18n翻譯、待辦事項、蕃茄鐘、井字遊戲',
        github: 'https://github.com/wuzhe0912/vue-tools-app',
        link: 'https://vue-tools-app.netlify.app/',
        tags: ['Vue.js', 'FlexBox'],
      },
      {
        title: 'Vue.js 聊天 App',
        layout: '手刻 Flexbox',
        api: '串接 Firebase 註冊登入',
        feature: '仿聊天app，提供聊天室、會員列表、個人聊天等功能',
        github: 'https://github.com/wuzhe0912/vue-firebase-chat',
        link: 'https://vue-firebase-chat.netlify.app/',
        tags: ['Vue.js', 'Firebase'],
      },
      {
        title: 'Vue.js 仿 後台 CMS',
        layout: 'Element UI',
        api: '六角提供後台 API',
        feature: '主要用來練手後台，功能相對陽春，日後再抽時間將功能遷移到新 demo',
        github: 'https://github.com/wuzhe0912/vue-shope-admin',
        link: 'https://vue-shope-admin.netlify.app/',
        tags: ['Vue.js', 'Element UI'],
      },
    ],
  },
  // {
  //   id: 1,
  //   tag: 'JavaScript',
  //   list: [
  //     {
  //       title: 'Vanilla JS title',
  //       desc: `Vanilla JS desc`,
  //       github: '',
  //       link: 'https://www.oxxostudio.tw/articles/201908/js-array.html',
  //       tags: ['JavaScript'],
  //     },
  //   ],
  // },
  {
    id: 2,
    tag: 'Nuxt.js',
    list: [
      {
        title: 'Nuxt.js Blog',
        layout: '手刻 Flexbox',
        api: 'Storyblok、OpenWeather',
        feature: '一個簡易練手用 Blog Website',
        github: 'https://github.com/wuzhe0912/nuxt-blog',
        link: 'https://activello-nuxt-blog.netlify.app',
        tags: ['Nuxt.js'],
      },
    ],
  },
  {
    id: 3,
    tag: 'Hexo',
    list: [
      {
        title: 'Hexo Blog',
        layout: 'Next Theme',
        api: 'Hexo',
        feature: `
                入行後第一次使用靜態站點架設 Blog，採用很普遍的 Hexo 框架，中文化友善，
                因此無論是架設或是查詢問題，都可以很快獲得解決，Next 作為網站的風格，也相當簡潔具有質感，
                雖然陸續更新過筆記，但目前疏於維護，且從文件的角度來看，有時查閱內容，不如樹狀結構更為快速直覺
                ，目前正逐步遷移到新的文件工具 Docsify
              `,
        github: 'https://github.com/wuzhe0912/pittwu-blog',
        link: 'https://pittwu-blog.netlify.app/',
        tags: ['Hexo', '筆記', '靜態站點'],
      },
    ],
  },
  // {
  //   id: 4,
  //   tag: 'D3.js',
  //   list: [
  //     {
  //       title: 'D3.js title',
  //       desc: `D3.js desc`,
  //       github: '',
  //       link: '',
  //       tags: ['D3.js'],
  //     },
  //     {
  //       title: 'D3.js title',
  //       desc: `D3.js desc`,
  //       github: '',
  //       link: '',
  //       tags: ['D3.js'],
  //     },
  //   ],
  // },
  // {
  //   id: 5,
  //   tag: 'HTML&CSS',
  //   list: [
  //     {
  //       title: 'CSS Animation',
  //       desc: `記錄一些簡單的動畫實作方式。`,
  //       github: 'https://github.com/wuzhe0912/animation-effect-demo',
  //       link: 'https://animate-demo.netlify.app/',
  //       tags: ['HTML&CSS', 'Animation'],
  //     },
  //   ],
  // },
]